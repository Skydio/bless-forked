...
class BlessUserSchema(Schema):
    requesting_user = fields.Str(required=True)
    ...
    def validate_remote_usernames(self, remote_usernames, **kwargs):
...
class BlessUserRequest:
    def __init__(self, requesting_user, public_key_to_sign,
                 remote_usernames, command=None, kmsauth_token=None):
        """
        A BlessRequest must have the following key value pairs to be valid.
        :param requesting_user: The user requesting the certificate.  
        Encoded in the certificate for auditing purposes.
